...
const isSingalBookTransection = (value: any) => {
    ...
    if ( !value.bookId || typeof value.bookId !== 'string') {
        throw new Error('field books.bookId should be string');
    }
        
    if ( !value.bookNumber || typeof value.bookNumber !== 'number' || value.bookNumber <= 0) {
...
export function registerServiceTransection(): Router{
    ...
    router.post('/', 
        json(), [
            ...
            body("bookInfo").isArray().custom(
                (value) => {

                    if(!value) {
                        throw new Error('field bookInfo should not be empty');
                    }
